/* ==================================================================================
 Hurdle NB DLS Model - Multiplicative + Geographical Distance (in Hurdle component)
 ==================================================================================== */
...
inline void logistic_hurdle_eta(double eta,
                                double &pi_ij,
                                double &log_pi,
                                double &log1m_pi) {
  pi_ij = 1.0 / (1.0 + std::exp(-eta));
  if (pi_ij < 1e-12)     pi_ij = 1e-12;
  if (pi_ij > 1 - 1e-12) pi_ij = 1 - 1e-12;
  
  log_pi   = std::log(pi_ij);
  log1m_pi = std::log(1.0 - pi_ij);
}
...
double hurdle_loglik_cell(double y,
  ...
  double logM = alpha_t * ((gamma_i + theta_j) / 2.0) - d_ij;
  ...
  double one_minus_f0 = 1.0 - std::exp(logf0);
...
double mh_update_alpha(const arma::mat& Y,
  ...
  double ll_old = hurdle_loglik_one_year(Y, D, G, W,
  ...
  double ll_new = hurdle_loglik_one_year(Y, D, G, W,
  ...
  if (std::log(R::runif(0.0, 1.0)) < log_acc) return alpha_new;
  return alpha_t;
}
...
double mh_update_beta(const arma::mat& Y,
  ...
  double log_acc = (ll_new + lp_new) - (ll_old + lp_old);
  if (std::log(R::runif(0.0, 1.0)) < log_acc) return beta_new;
  return beta_t;
}

double mh_update_rho(const arma::mat& Y,
                     const arma::mat& D,
                     const arma::mat& G,
                     const arma::mat& W,
                     double rho_t,
  ...
  double ll_old = hurdle_loglik_one_year(Y, D, G, W,
                                         ...
                                         gamma_vec, theta_vec,
                                         include_diagonal);
  double lp_old = -0.5 * std::pow(rho_t - mu_rho, 2) / sigma2_rho;
  ...
  double ll_new = hurdle_loglik_one_year(Y, D, G, W,
                                         ...
                                         gamma_vec, theta_vec,
                                         include_diagonal);
  double lp_new = -0.5 * std::pow(rho_new - mu_rho, 2) / sigma2_rho;
  
  double log_acc = (ll_new + lp_new) - (ll_old + lp_old);
  if (std::log(R::runif(0.0, 1.0)) < log_acc) return rho_new;
  return rho_t;
}
...
double mh_update_r_halfnormal(const arma::mat& Y,
  ...
  if (std::log(R::runif(0.0, 1.0)) < log_acc) return r_new;
  return r_t;
}
...
arma::vec mh_update_r_joint_halfnormal(
  ...
  arma::vec r_new(Tn), a_old(Tn), a_new(Tn);
  ...
  for (int t=0; t<Tn; t++) {
    ll_old += hurdle_loglik_one_year(
      Y_list[t], D_list[t], G_list[t], W_list[t],
...
bool mh_update_Xi(const arma::mat& Y,
  ...
  for (int j = 0; j < n; ++j) {
    ...
    double pi_ij, log_pi_ij, log1m_pi_ij;
    logistic_hurdle_eta(beta_t + rho_t * G(i,j), pi_ij, log_pi_ij, log1m_pi_ij);
    
    double pi_ji, log_pi_ji, log1m_pi_ji;
    logistic_hurdle_eta(beta_t + rho_t * G(j,i), pi_ji, log_pi_ji, log1m_pi_ji);
    
    ll_old += hurdle_loglik_cell(y_ij, d_ij, alpha_t, r_t,
                                 gamma_vec(i), theta_vec(j),
                                 log_pi_ij, log1m_pi_ij);
    
    ll_old += hurdle_loglik_cell(y_ji, d_ij, alpha_t, r_t,
  ...
  for (int j = 0; j < n; ++j) {
    ...
    ll_new += hurdle_loglik_cell(y_ij, dij_new, alpha_t, r_t,
                                 gamma_vec(i), theta_vec(j),
                                 log_pi_ij, log1m_pi_ij);
    
    ll_new += hurdle_loglik_cell(y_ji, dij_new, alpha_t, r_t,
                                 gamma_vec(j), theta_vec(i),
                                 log_pi_ji, log1m_pi_ji);
  }
...
int mh_update_X_year(const arma::mat& Y,
  ...
  for (int i = 0; i < n; ++i) {
    bool ok = mh_update_Xi(Y, X_list, D_t, t_idx, i, G,
                           alpha_t, beta_t, rho_t, r_t,
...
double loglik_gamma_i_allT(const std::vector<arma::mat>& Y_list,
  ...
  for (int t = 0; t < Tn; ++t) {
    ...
    double r_t     = r_vec(t);
    double gamma_i = gamma_vec(i_node);
    ...
    for (int j = 0; j < n; ++j) {
      ...
      if (W(i_node, j) != 0) {
        ...
        out += hurdle_loglik_cell(y_ij, d_ij,
                                  alpha_t, r_t,
                                  gamma_i, theta_vec(j),
                                  log_pi, log1m_pi);
      }
      ...
      if (W(j, i_node) != 0) {
        ...
        out += hurdle_loglik_cell(y_ji, d_ij,
                                  alpha_t, r_t,
...
double mh_update_gamma_i_allT(const std::vector<arma::mat>& Y_list,
  ...
  double ll_old = loglik_gamma_i_allT(Y_list, D_list, G_list, W_list,
  ...
  double ll_new = loglik_gamma_i_allT(Y_list, D_list, G_list, W_list,
                                      alpha_vec, beta_vec, rho_vec, r_vec,
                                      gamma_vec, theta_vec,
                                      i_node, include_diagonal);
  ...
  if (std::log(R::runif(0.0,1.0)) < log_acc) return g_new;
...
double mh_update_theta_j_allT(const std::vector<arma::mat>& Y_list,
  ...
  double ll_old = loglik_theta_j_allT(Y_list, D_list, G_list, W_list,
                                      alpha_vec, beta_vec, rho_vec, r_vec,
                                      gamma_vec, theta_vec,
  ...
  double ll_new = loglik_theta_j_allT(Y_list, D_list, G_list, W_list,
                                      alpha_vec, beta_vec, rho_vec, r_vec,
                                      gamma_vec, theta_vec,
                                      j_node, include_diagonal);
  ...
  if (std::log(R::runif(0.0,1.0)) < log_acc) return th_new;
...
/* ==============================================================
 ...
 ============================================================== */
// [[Rcpp::export]]
List mcmc_moltiplicativo_geo(List Y_list_R,
                             List X_list_R,
                             List W_list_R,
                             arma::mat G,              
  ...
  for (int t = 0; t < Tn; ++t) D_list[t] = pairwise_dist(X_list[t]);
  
  std::vector<arma::mat> G_list(Tn);
  for (int t = 0; t < Tn; ++t) G_list[t] = G;
  
  if (sender_ref >= 0 && sender_ref < n_nodes)   gamma_vec(sender_ref) = 0.0;
  if (receiver_ref >= 0 && receiver_ref < n_nodes) theta_vec(receiver_ref) = 0.0;
  ...
  for (int iter = 1; iter <= n_iter; ++iter) {
    ...
    for (int t = 0; t < Tn; ++t) {
      ...
      double newv = mh_update_alpha(Y_list[t], D_list[t], G_list[t], W_list[t],
                                    oldv, beta_vec(t), rho_vec(t), r_vec(t),
                                    gamma_vec, theta_vec,
                                    mu_alpha, sigma2_alpha,
                                    prop_sd_alpha, include_diagonal);
      if (newv != oldv) acc_alpha(t)++;
      trials_alpha(t)++;
      alpha_vec(t) = newv;
    }
    ...
    for (int t = 0; t < Tn; ++t) {
      ...
      double newv = mh_update_beta(Y_list[t], D_list[t], G_list[t], W_list[t],
                                   oldv, alpha_vec(t), rho_vec(t), r_vec(t),
                                   gamma_vec, theta_vec,
                                   mu_beta, sigma2_beta,
                                   prop_sd_beta, include_diagonal);
      if (newv != oldv) acc_beta(t)++;
      trials_beta(t)++;
      beta_vec(t) = newv;
    }
    ...
    for (int t = 0; t < Tn; ++t) {
      ...
      double newv = mh_update_rho(Y_list[t], D_list[t], G_list[t], W_list[t],
                                  oldv, alpha_vec(t), beta_vec(t), r_vec(t),
                                  gamma_vec, theta_vec,
                                  mu_rho, sigma2_rho,
                                  prop_sd_rho, include_diagonal);
      if (newv != oldv) acc_rho(t)++;
      trials_rho(t)++;
      rho_vec(t) = newv;
    }
    ...
    if (iter <= burn_in && (iter % 100 == 0)) {
      ...
      for (int t = 0; t < Tn; ++t) {
        double ar_a = (double)acc_alpha(t) / trials_alpha(t);
        if (ar_a < 0.3) prop_sd_alpha *= 0.8;
        else if (ar_a > 0.6) prop_sd_alpha *= 1.2;
        
        double ar_b = (double)acc_beta(t) / trials_beta(t);
        if (ar_b < 0.3) prop_sd_beta *= 0.8;
        else if (ar_b > 0.6) prop_sd_beta *= 1.2;
        
        double ar_rh = (double)acc_rho(t) / trials_rho(t);
        ...
        else if (ar_X > 0.5) prop_sd_X *= 1.2;
        
        double ar_r = (double)acc_r(t) / trials_r(t);
        if (ar_r < 0.2) prop_sd_r *= 0.8;
        else if (ar_r > 0.5) prop_sd_r *= 1.2;
    ...
    if (iter > burn_in) {
      ...
      List X_this_iter(Tn);
      for (int tt = 0; tt < Tn; ++tt) X_this_iter[tt] = X_list[tt];
  ...
  List X_out(Tn);
  for (int t = 0; t < Tn; ++t) X_out[t] = X_list[t];
