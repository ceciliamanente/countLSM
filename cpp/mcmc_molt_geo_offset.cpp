/* ================================================================================
 Hurdle NB DLS Model - Multiplicative + Geographical Distance + Exposure Adj. Term  
 ================================================================================== */
...
/* ==============================================================
 ...
 ============================================================== */
// [[Rcpp::export]]
List mcmc_molt_geo_offset(List Y_list_R,
                          List X_list_R,
                          List W_list_R,
                          arma::mat G,
                          List logE_list_R,
  ...
  for (int iter = 1; iter <= n_iter; ++iter) {
    ...
    if (iter <= burn_in && (iter % 100 == 0)) {
      ...
      for (int i = 0; i < n_nodes; ++i) {
        double ar_g = (double)acc_gamma(i) / trials_gamma(i);
        if (ar_g < 0.3) prop_sd_gamma *= 0.8;
        else if (ar_g > 0.6) prop_sd_gamma *= 1.2;
        
        double ar_th = (double)acc_theta(i) / trials_theta(i);
        if (ar_th < 0.3) prop_sd_theta *= 0.8;
        else if (ar_th > 0.6) prop_sd_theta *= 1.2;
        
        acc_gamma(i) = trials_gamma(i) = 0;
        acc_theta(i) = trials_theta(i) = 0;
      }
    }
