/* =============================================================================
 Hurdle NB DLS Model - Baseline + Geographical Distance (in Hurdle componente)
 =============================================================================== */
...
double hurdle_loglik_one_year(const arma::mat& Y,
  ...
  for (int i = 0; i < n; ++i) {
    for (int j = 0; j < n; ++j) {
      ...
      ll += hurdle_loglik_cell(y, dij, alpha_t, r_t, log_pi, log1m_pi);
    }
  }
  return ll;
}
...
double mh_update_alpha(const arma::mat& Y,
  ...
  double ll_old = hurdle_loglik_one_year(Y, X, D, G, W,
  ...
  double ll_new = hurdle_loglik_one_year(Y, X, D, G, W,
...
bool mh_update_Xi(const arma::mat& Y,
  ...
  for (int j = 0; j < n; ++j) {
    ...
    ll_old += hurdle_loglik_cell(y_ij, d_ij, alpha_t, r_t, log_pi, log1m_pi);
    ll_old += hurdle_loglik_cell(y_ji, d_ji, alpha_t, r_t, log_pi, log1m_pi);
  }
  ...
  for (int j = 0; j < n; ++j) {
    ...
    ll_new += hurdle_loglik_cell(y_ij, dij_new, alpha_t, r_t, log_pi, log1m_pi);
    ll_new += hurdle_loglik_cell(y_ji, dij_new, alpha_t, r_t, log_pi, log1m_pi);
  }
...
arma::vec mh_update_r_joint_halfnormal(
  ...
  for (int t=0; t<Tn; t++) {
    ll_old += hurdle_loglik_one_year(
      Y_list[t], X_list[t], D_list[t], G_list[t], W_list[t],
...
/* ==============================================================
 ...
 ============================================================== */
// [[Rcpp::export]]
List mcmc_base_geo(List Y_list_R,
             List X_list_R,
             List W_list_R,
             arma::mat G,              
  ...
  for (int iter = 1; iter <= n_iter; ++iter) {
    ...
    for (int t = 0; t < Tn; ++t) {
      ...
      double rho_new = mh_update_rho(Y_list[t],
                                     X_list[t],
                                           D_list[t],
                                                 G_list[t],
                                                       W_list[t],
    ...
    if (joint_update_r) {
      ...
      arma::vec r_new_vec =
        mh_update_r_joint_halfnormal(
          Y_list, X_list, D_list, G_list, W_list,
          r_vec,
          alpha_vec,
          beta_vec,
          rho_vec,
