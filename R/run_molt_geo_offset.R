# ==============================================================================
# ==============================================================================
# ======== Multiplicative + Geographical Distance + Exp.Adj. Term Model ========
# ==============================================================================
# ==============================================================================
...
# PATHS 
# ---------------------------------------------------------------
cpp_path        <- "path/to/cpp/mcmc_molt_geo_offset.cpp"
...
# ---------------------------------------------------------------
# 1c). GEOGRAPHICAL DISTANCES MATRIX 
# ---------------------------------------------------------------
...
init_beta <- function(Y_list) {
  sapply(Y_list, function(Y) {
    nz <- mean(Y > 0)
    nz <- pmax(pmin(nz, 1 - 1e-6), 1e-6)  # clamp
    qlogis(nz)
  })
}
...
res_molt_geo_offset <- mcmc_molt_geo_offset(
  Y_list_R = Y_list,
  X_list_R = X_list,
  W_list_R = W_list,
  G = G,                   
...
saveRDS(res_molt_geo_offset, file = file.path(output_dir_mcmc, "res_molt_geo_offset.rds"))

# ---------------------------------------------------------------
# 5). DIAGNOSTIC
# ---------------------------------------------------------------

res_molt_geo_offset$DIC

alpha_samples <- res_molt_geo_offset$alpha_samples
beta_samples <- res_molt_geo_offset$beta_samples
rho_samples <- res_molt_geo_offset$rho_samples   
r_samples <- res_molt_geo_offset$r_samples
X_last <- res_molt_geo_offset$X_last
X_samples <- res_molt_geo_offset$X_samples
gamma_samples <- res_molt_geo_offset$gamma_samples
theta_samples <- res_molt_geo_offset$theta_samples
...
ppc_results_molt_geo_offset <- ppc_all_years_MOLT_GEO_OFFSET(
  Y_list = Y_list,
...
saveRDS(ppc_results_molt_geo_offset, file = file.path(output_dir_ppc, "ppc_results_molt_geo_offset.rds"))
...
for (year in names(Y_list)) {
  ...
  plot_ppc_stats(
    ppc_results = ppc_results_molt_geo_offset,
    year_name = year,
    model = "base"
  )
