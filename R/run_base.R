...
# PATHS 
# ---------------------------------------------------------------
cpp_path      <- "path/to/cpp/mcmc_base.cpp"
...
init_X_mds <- function(Y_list, p = 2) {
  ...
  for (tt in seq_len(TT)) {
    ...
    # Allinea con anno precedente (Procrustes)
...
alpha_samples <- res_base$alpha_samples
beta_samples <- res_base$beta_samples
r_samples <- res_base$r_samples
...
ppc_results_base <- ppc_all_years_BASE(
  Y_list = Y_list,
  res = res_base,
  W_list = W_list,
  n_sim = 500,
...
# Save
saveRDS(ppc_results_base, file = file.path(output_dir_ppc, "ppc_results_base.rds"))
